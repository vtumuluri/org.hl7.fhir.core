...
public abstract class Base implements Serializable, IBase {
    ...
    public boolean hasType(String... name) {
        String t = fhirType();
        for (String n : name) if (n.equals(t))
            return true;
        return false;
    }
    ...
    public Property getChildByName(String name) {
        ...
        for (Property c : children) if (c.getName().equals(name))
            return c;
        return null;
    }
    ...
    public List<Base> listChildrenByName(String name) {
        ...
        if (name.equals("*")) {
            ...
            for (Property c : children) if (c.getName().equals(name) || (c.getName().equals(name + "[x]")))
                return c.getValues();
        }
    ...
    public static boolean compareDeep(Base e1, Base e2, boolean allowNull) {
        ...
        if (e1 == null || e2 == null)
            return false;
        if (// respect existing order for debugging consistency; outcome must be the same either way
        e2.isMetadataBased() && !e1.isMetadataBased())
